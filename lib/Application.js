...
Application.prototype.oscSync = function(port) {
  ...
  let time_sync = this.time_sync = { type: 'oscSync' }
  let oscServer = new osc.Server(port || 7031, '0.0.0.0');
  let position  = time_sync.position = new Continuous

  oscServer.on("message", function (msg, rinfo) {
    let url = msg[0]
    let re  = SONG_POS_RE.exec(url)
    if (re) {
      position.setValue(parseInt(re[1]), elapsed() + this.sync_delta)
    } else if (url === '/sync/song') {
      position.setValue(msg[1], elapsed() + this.sync_delta)
    } else if (url === '/sync/envelope') {
      // we use 8 beats per track
      position.setValue(msg[1] * 32, elapsed() + this.sync_delta)
...
Application.prototype.setTime = function(s) {
  ...
  if (time_sync) {
    this.bpm  = time_sync.bpm
    this.song = time_sync.position.value(s)
    this.time = s
  } else {
    this.song = this.bpm / 60 * s
    this.time = s
  }
}
