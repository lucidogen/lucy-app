...
{ let self = this
  ...
  // If source does not contain a dirsum (older version, compute it now)
  let sourceInfoPath = source + '/snapshot.json'
  let info
  if ( !statPath ( sourceInfoPath ) )
  { info = { time: 0 }
  }
  else
  { info = JSON.parse ( fs.readFileSync ( sourceInfoPath ) )
  }
  if ( !info.checksum )
  {
    dirsum
    ...
    , function ( err, hex )
      { if ( err )
        { alert ( `Could not compute scene checksum (${ err }).`)
        }
        else
        { info.checksum = hex
          fs.writeFile
          ...
          , function ( err )
            { if ( err )
              { alert
                ( `Could not write snapshot.json in scene path (${ err }).` )
              }
              else
              { // restart
                self.edit ( scene_path )
              }
            }
          )
        }
  ...
  if ( lastsum == '' )
  { let workcontent =
    fs.readdirSync ( self.work_path )
    .filter
    ( function ( e )
      { return ! IGNORE_FUNCTION ( e, e )
      }
    )
